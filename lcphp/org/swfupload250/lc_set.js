...
function uploadSuccess(file, serverData) {
    ...
    if(serverData.state!='SUCCESS'){//上传错误
        ...
        var msg ='';//提示文本
        msg = '你已经上传'+file_upload.file_success_nums+'个文件，还可以上传'+(file_upload.file_allow_total-file_upload.file_nums)+'个文件，每个不超过'+file_upload.swfuobj.settings.file_size_limit;
    ...
    $.ajax({
        ...
        success:function(data){
            if(data==1){
                ...
                msg = '你已经上传'+swfu_data.file_success_nums+'个文件，还可以上传'+(swfu_data.file_allow_total-swfu_data.file_nums)+'个文件，每个不超过'+swfu_data.swfuobj.settings.file_size_limit;
                $("#swfupload_message"+swfupload_id).html(msg).addClass("swfupload_message_style");//输出上传状态文本
            }else{
                alert("删除失败");
            }
    ...
    try {
        ...
        switch (errorCode) {
            case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                progress.setStatus("文件太大。"+message);
