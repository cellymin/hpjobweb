...
* @Description: todo(关联模型)
* @author zhouchao
* @date 2014-12-30 上午8:36:44
 */
class relationModel extends Model {

    public $join = array(); //关联数据
    private $relation = array(); //关联模型数据，涵盖，主表，次表，显示字段，筛选方法(如where)等
    private $join_model = array(); //只关联的模型

    /**
     * 是否执行关联
     * @param type $type    关联方式 true 匹配所有关联（默认值）
     */

    function __init() {
        if ($this->join_model === false) {//没有任何模型操作
            return false;
        }
        if (empty($this->join_model)) {//如果通过join()类方法，设置了要关联的模型时执行验证
            $this->join_model = array_keys(array_change_key_case_d($this->join, 1));
        }
        foreach ($this->join as $table => $args) {
            if (!is_array($args)) {
                error(L("relationmodel_check_error0"), false);
            }
            if (!in_array(strtoupper($table), $this->join_model)) {//关联的模型时执行验证
                continue;
            }
            ...
            $args['son_foreign_key'] = $this->get_foreign_key($args); //获得从键
            $args['parent_table'] = strtoupper($args['type'] == 'BELONGS_TO') ? C("DB_PREFIX") . $table : $this->tableName; //获得主表
            $args['parent_key'] = $this->get_parent_key($args); //获得主表的关联键
            $args['relation_cmma_method'] = $this->relation_cmma_method($args); //从表中的count,max,min
    ...
    private function check($args) {
        if (!isset($args['type']) || empty($args['type'])) {
            error(L("relationmodel_check_error1")); //多表操作定义的表的模型属性type值没有定义
        }
        $args['type'] = strtoupper($args['type']);
        if (!in_array($args['type'], array("HAS_ONE", "HAS_MANY", "BELONGS_TO", "MANY_TO_MANY"))) {
            error(L("relationmodel_check_error2")); //多表操作定义的表的模型属性type值必须是HAS_ONE、HAS_MANY、BELONGS_TO、MANY_TO_MANY中的一个，不区分大小写
        }
        if ($args['type'] == 'MANY_TO_MANY') {
            if (!isset($args['relation_table'])) {
                error(L("relationmodel_check_error3")); //使用多表操作MANY_TO_MANY的表没有定义relation_table属性即中间关联表
            }
            if (!isset($args['relation_table_parent_key']) || empty($args['relation_table_parent_key'])) {
                error(L("relationmodel_check_error4")); //使用多表操作MANY_TO_MANY的表没有定义relation_table_parent_key属性
            }
            if (!isset($args['relation_table_foreign_key']) || empty($args['relation_table_foreign_key'])) {
                error(L("relationmodel_check_error5")); //使用多表操作MANY_TO_MANY的表没有定义relation_table_foreign_key属性
            }
        }
    }

    //关联表的模型count,max,min,avg方法
    private function relation_cmma_method($args) {
    ...
    //从表的SQL查询参数配置如field、where等
    private function relation_method($table, $args) {
        $db = M($table);
    ...
    //获得主表的关联键
    private function get_parent_key($args) {
    ...
    //获得关联外键
    private function get_foreign_key($args) {
        ...
        error(L("relationmodel_get_foreign_key2")); //定义关联模型必须指定foreign_key值
    }
    ...
    protected function reset() {
        $this->db->opt_init(); //主模型查询参数归位
        $this->join_model = array(); //关联模型复位
    }
    ...
    public function select() {
        ...
        foreach ($parent_result as $k => $v) {
            ...
            foreach ($this->relation as $table => $args) {
                $db = M($table); //关联表表模型
                $this->relation_method($table, $args); //执行从表SQL如field、where
                switch ($args['type']) {
                    case "HAS_ONE":
                    case "HAS_MANY":
                        ...
                        if (!empty($args['relation_cmma_method'])) {//执行模型定义的CURD 如count max avg
                            foreach ($args['relation_cmma_method'] as $methodName => $methodArgs) {
                    ...
                    case "BELONGS_TO":
                        ...
                        if (!empty($args['relation_cmma_method'])) {//执行模型定义的CURED 如count max avg
                            foreach ($args['relation_cmma_method'] as $methodName => $methodArgs) {
                    ...
                    case "MANY_TO_MANY":
                        ...
                        if (!empty($relation_table_info)) {
                            foreach ($relation_table_info as $foreign_value) {
                                ...
                                $result[$k][$args['son_table']][] = current($relation_result);
                            }
                            continue 2;
                        } else {
                            if (!$args['other']) {//如果没有关联结果集，且设置不显示关联不成功数据时删除查找到的数据
    ...
    public function insert() {
        ...
        $insert_data = $this->get_args(func_get_args());
        if (empty($insert_data)) {
            if (!empty($_POST)) {
                $insert_data = array($_POST);
            } else {
                error(L("relationmodel_insert")); //悲剧了。。。执行INSERT()时没有任何插入数据，插入数据可以是$_POST也可以直接传入INSERT()方法中
            }
        }
        $this->__init();
        ...
        if ($this->join_model === false || !$model_insert_id) {//没有任何操作模型
    ...
    public function update() {
        ...
        $update_data = $this->get_args(func_get_args());
        if (empty($update_data)) {
            if (!empty($_POST)) {
                $update_data = array($_POST);
            } else {
                error(L("relationmodel_update")); //悲剧了。。。执行INSERT()时没有任何插入数据，插入数据可以是$_POST也可以直接传入INSERT()方法中
            }
        }
        $this->__init();
        $modelTableName = preg_replace("/" . C("DB_PREFIX") . "/", '', $this->tableName); //模型表名去表前缀
        ...
        if ($this->join_model === false) {//没有任何操作模型
            return $model_update_affected; //主表受影响的记录
        }
        $model_db = M($this->tableName, true); //主模型表对象
        $model_db->db->opt = $model_db->db->opt_old; //主模型更新时的条件
