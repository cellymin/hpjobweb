 ...
 * 
* @ClassName: mysql
* @Description: todo(mysql数据操作)
* @author zhouchao
* @date 2014-12-30 上午8:34:05
 */
...
//将查询SQL压入调试数组
    ...
    public function insert($opt) {
        ...
        $opt = $this->formatArgs($opt);
        if ($opt === false) {
            error(L("mysql_insert_error"), false); //'没有任何数据要插入,系统会将$_POST值自动插入，也可以手动将数据传入或者用ORM方式'
        }
        $opt = $opt[0];
        ...
        foreach ($opt as $k => $data) {
            ...
            if (empty($value)) {
                error(L("mysql_insert_error2"), false); //"插入数据错误，原因可能为1：插入内容为空   2：字段名非法"
            }
    ...
    public function replace($opt) {
        $opt = $this->formatArgs($opt);
        if ($opt === false) {
            error(L("mysql_replace_error1"), false); //没有任何数据要插入,系统会将$_POST值自动插入，也可以手动将数据传入或者用ORM方式
        }
        $opt = $opt[0];
        if (!is_array(current($opt))) {
            $opt = array($opt);
        ...
        foreach ($opt as $k => $data) {
            ...
            $value = $this->formatField($data);
            if (empty($value)) {
                error(L("mysql_replace_error2"), false); //插入数据错误，原因可能为1：插入内容为空   2：字段名非法！
            }
    ...
    public function update($opt) {
        ...
        $opt = $this->formatArgs($opt);
        if ($opt === false) {
            error(L("mysql_update_error1"), false); //没有任何数据要更新,系统会将$_POST值自动更新，也可以手动将数据传入或者用ORM方式
        }
        $opt = $opt[0];
        if (!is_array(current($opt))) {
            $opt = array($opt);
    ...
    public function delete($opt) {
        ...
        if (empty($this->opt['where'])) {
            error(L("mysql_delete_error"), false); //DELETE删除语句必须输入条件,如果删除数据有表的主键字段也可以做为条件使用
        }
...
AND ";
    ...
    public function in($opt) {
        $opt = $this->formatArgs($opt);
        if ($opt === false) {
            error(strtoupper(__FUNCTION__) . L("mysql_in_error"), false); //的参数不能为空
        }
    ...
    public function limit($opt) {
        $opt = $this->formatArgs($opt);
        if ($opt === false) {
            error(strtoupper(__FUNCTION__) . L("mysql_limit_error"), false); //的参数不能为空
        }
        foreach ($opt as $v) {
            if (is_array($v)) {
                $this->limit($v);
    ...
    public function order($opt) {
        $opt = $this->formatArgs($opt);
        if ($opt === false) {
            error(strtoupper(__FUNCTION__) . L("mysql_order_error"), false); // 的参数
        }
        foreach ($opt as $k => $v) {
            if (is_array($v)) {
                $this->order($v);
    ...
    public function group($opt) {
        $opt = $this->formatArgs($opt);
        if ($opt === false) {
            error(strtoupper(__FUNCTION__) . L("mysql_group_error"), false); // 的参数
        }
        foreach ($opt as $v) {
            if (is_array($v)) {
                $this->group($v);
