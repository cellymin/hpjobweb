 ...
 * 
* @ClassName: BaseTag
* @Description: todo(基本标签类)
* @author zhouchao
* @date 2014-12-30 上午8:53:08
 */
class baseTag {
...
//block 块标签          1为块标签  0独立标签 
    ...
    public function _foreach($attr, $content) {
        if (empty($attr['from'])) {
            error(L("baseTag__upload1"), false); //foreach 模板标签必须有from属性
        }
        if (empty($attr['value'])) {
            error(L("baseTag__upload2"), false); //foreach 模板标签必须有value属性
        }
        $php = ''; //组合成PHP
        $from = $attr['from'];
    ...
    public function _load($attr, $content) {
        if (!isset($attr['file'])) {
            error(L("baseTag__load"), false); //load标签必须有file属性
        }
        $file = $attr['file'];
        $preg = array(
            '/__TPL__/',
    ...
    public function _if($attr, $content, $res) {
        if (empty($attr['value'])) {
            error(L("baseTag__if"), false); //if 模板标签必须有value属性
        }
        $value = $attr['value'];
        $php = ''; //组合成PHP
        $php.='<?php if(' . $value . '){?>';
    ...
    public function _while($attr, $content, $res) {
        if (empty($attr['value'])) {
            error(L("baseTag__while"), false); //while模板标签必须有value属性
        }
        $value = $attr['value'];
        $php = ''; //组合成PHP
        $php.='<?php ' . " while($value){ ?>";
    ...
    public function _empty($attr, $content, $res) {
        if (empty($attr['value'])) {
            error(L("baseTag__empty"), false); //empty模板标签必须有value属性
        }
        $value = $attr['value'];
        $php = "";
    ...
    protected function _ueditor($attr, $content)
    {
        ...
        if (! isset($attr['name'])) {
            error(L("LCbasetag__editor"), false);
        }
        $name = $attr['name'];
        $str = '';
        $toole = '';
        ...
        switch ($attr['style']) {
            case 1:
                ...
                break;
            case 2:
                $toole = "toolbars:[['Source', 'Undo', 'Redo','Bold','Italic','Underline','JustifyLeft','JustifyCenter','JustifyRight','InsertOrderedList','InsertUnorderedList','FormatMatch','Link','Horizontal']],";
                break;
        }

        $path = __lcphp__ . '/org/ueditor';
        $str .= '<script type="text/javascript">LC_UEDITOR_ROOT="' . __lcphp__ . '/org/ueditor/";</script>';
        ...
        $str .= '<script type="text/plain" name="' . $name . '" id="' . $id . '" ' . $width . '>' . $content . '</script>';
        $str .= '<script type="text/javascript" >';
    ...
    {
        if (! isset($attr['name'])) {
            error(L("basetag__upload"), false); // 上传标签upload必须指定name属性，如果不清楚使用规范请查看后盾LC框架手册
        }
...
<script type='text/javascript' src='" . $swfupload_path . "/js/fileprogress.js'></script>
  <script type='text/javascript' src='" . $swfupload_path . "/lc_set.js'></script>
";
        $str .= '<?php endif;?>';

        $str .= file_get_contents(PATH_LC . '/org/swfupload250/lc_set.php');
        $str .= "
            <!--上传显示DIV-->
