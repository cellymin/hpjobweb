 ...
 * 
* @ClassName: compile
* @Description: todo(框架编译类)
* @author zhouchao
* @date 2014-12-30 上午8:52:38
 */
class compile extends lcphp {
    ...
    public function run() {
        ...
        $this->content = '<?php if(!defined("PATH_LC"))exit;?>' . "\r\n" . $this->content;
...
//    }
    ...
    private function loadParseTags() {
        ...
        load_file(PATH_LC . '/libs/usr/view/base/baseTag.class.php'); //加载基标签库
        $tagClass[] = 'baseTag';
    ...
    private function removeEmpty($content) {
        $content = preg_replace('/\$base.(\w+)\./ise', '\'\$_\'.strtoupper("\1").".";', $content);
        $preg = array(
            '/[{}]/',
            '/\s*\|\s*/',
            '/\s*:\s*/',
