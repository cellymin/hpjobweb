...
final class application {
    ...
    static private function loadUserFile() {
        C(include(PATH_LC . '/libs/boot/usrFiles.php')); //将系统核心类路径压入配置文件中
    }
    ...
    static private function init() {//项目运行初始化
        C(include PATH_LC . '/libs/boot/config.php'); //加载框架核心配置文件
        ...
        C(require (PATH_LC . '/libs/boot/config.php')); //载入核心配置文件
        if (function_exists("spl_autoload_register")) {
            spl_autoload_register(array(__CLASS__, "autoload")); //注册自动载入函数
        }
        set_error_handler(array("exceptionLC", "error"), E_ALL); //设置错误处理函数
        set_exception_handler(array("exceptionLC", "exception")); //设置异常
...
str;
        ...
        copy(PATH_LC . '/libs/boot/config.php', $demoDir['demo_config_dir'] . '/config.php'); //复制错误页面error.html到模版文件夹下
        copy(PATH_LC . '/data/tpl/error.html', $demoDir['demo_tpl_public_dir'] . '/error.html'); //复制错误页面error.html到模版文件夹下
        copy(PATH_LC . '/data/tpl/success.html', $demoDir['demo_tpl_public_dir'] . '/success.html'); //复制错误页面error.html到模版文件夹下
