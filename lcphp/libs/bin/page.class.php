...
class page {
    ...
    function __construct($total, $row = '', $page_row = '', $style = '', $desc = '') {
        ...
        if(isset($_GET[C("PAGE_VAR")])){
            $_POST[C("PAGE_VAR")] = $_GET[C("PAGE_VAR")];
        }
        ...
        $this->self_page = min($this->total_page, empty($_POST[C("PAGE_VAR")]) ? 1 : max(1, (int) $_POST[C("PAGE_VAR")])); //当前页
    ...
    //bootstrap 上一页样式
    protected function bootspre() {
        if ($this->self_page > 1 && $this->self_page <= $this->total_page) {
            return "<li class='prev'><a href='" . $this->url . ($this->self_page - 1) . "'><<</a></li>";
            return "<a href='" . $this->url . ($this->self_page - 1) . "'>{$this->desc['pre']}</a>";
        }else{
            return "<li class='prev disabled'><a href='javascript:void(0)'><<</a></li>";
        }
        return;
    }

    //下一页
    public function bootsnext() {
        $next = $this->desc ['next'];
        if ($this->self_page < $this->total_page) {
            return "<li class='next '><a href='" . $this->url . ($this->self_page + 1) . "'>>></a></li>";
        }  else {
            return "<li class='next disabled'><a href='javascript:void(0)'>>></a></li>";
        }
        return;
    }
    ...
    public function bootlist(){
        ...
        if (empty($arr))
            return;
        foreach ($arr as $v) {
            $str .= empty($v ['url']) ? "<li class='active'><a href='#'>" . $v ['str'] . "</a></li>" : "<li><a href={$v['url']}>{$v['str']}</a></li>&nbsp;";
        }
        $str .= $this->bootsnext();
        return $str;
    }
    ...
    /**
     * @Title: nostylenext
     * @Description: todo(无样式下一页)
     * @author zhouchao
     * @return  void  返回类型
     */
    public function nostylenext(){
        if ($this->self_page < $this->total_page) {
            return $this->url . ($this->self_page + 1) ;
        }else{
            return "";
        }
    ...
    public function strlist() {
        $arr = $this->pagelist();
        $str = '';
        $first = $this->self_page == 1 ? "" : "<a href='{$this->url}1'><span class='current'>首页 </span></a>&nbsp;";
        $end = $this->self_page >= $this->total_page ? "" : "<a href='{$this->url}{$this->total_page}'><span class='current'>尾页</span></a>&nbsp;";
        if (empty($arr))
            return;
        foreach ($arr as $v) {
...
";
    ...
    public function show($s = '') {
        ...
        switch ($s) {
            ...
            case 4:
                return $this->bootlist();//Bootstrap分页样式
        }
    }
