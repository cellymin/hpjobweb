...
//附加词典
    ...
    static $addonDicFile = '/splitword/words_addons.dic';
//主词典
    static $dicStr = '';
    static $mainDic = array();
    static $mainDicHand = FALSE;
    static $mainDicInfos = array();
    static $mainDicFile = '/splitword/base_dic_full.dic';
//是否直接载入词典（选是载入速度较慢，但解析较快；选否载入较快，但解析较慢，需要时才会载入特定的词条）
    static $mainDicFileZip = '/splitword/base_dic_full.zip';
...
//词库载入时间
     ...
     */
    static public function split_word($string, $source_charset = '', $target_charset = 'utf-8', $load_all = TRUE) {
        $charset = C("CHARSET");
        $source_charset = empty($source_charset) ? preg_replace("/utf8|utf-8/i", "utf-8", $charset) : $source_charset;
        self::$addonDicFile = PATH_ORG . str_replace(PATH_ORG,'',self::$addonDicFile);
        self::$mainDicFileZip = PATH_ORG . str_replace(PATH_ORG,'',self::$mainDicFileZip);
        self::$mainDicFile = PATH_ORG . str_replace(PATH_ORG,'',self::$mainDicFile);
