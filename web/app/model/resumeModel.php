...
class resumeModel extends Model{
	...
	public function getResumes($cond)
	{
        ...
        $page=new page($nums,5);
        $resumes['resumes']=$db->where($cond)->findall($page->limit());
        $resumes['page']=$page->total_page;
        return $resumes;
	}
...
//        $page=new page($num,5);
        $works['exp']=$db->field('resume_id,work_exp_id,job_start,job_end,job_name,department,job_desc,company_name')->where($cond)->findall();
//        $works['page']=$page->total_page;
		return $works;
	}
