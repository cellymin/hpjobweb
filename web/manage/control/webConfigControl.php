...
class webConfigControl extends myControl {
    ...
    private function _setConfig($data=array(),$config='app')
    {
        ...
        $config = '<?php if (!defined("PATH_LC")) exit;return ' . str_replace(array("'TRUE'","'FALSE'"),array('TRUE','FALSE'),var_export(array_merge($config, $data), true)) . ';?>'; //新的配置文件
    ...
    function testEmail() {
        $email = new mail();
        if ($email->send($_POST['email'], '测试用户', '这是一封测试邮件！', '这是一封测试邮件！你看到此邮件，说明你的邮箱已经配置好了。欢迎来到' . '<a href="' . __WEB__ . '">' . C('WEB_NAME') . '</a><br /><a href="http://www.houdunwang.com"><img src="http://bbs.houdunwang.com/static/image/common/logo.png" alt="快乐PHP培训" /></a>')) {
            $this->success('邮件发送成功');
        }
        $this->success('邮件发送失败');
    }
    ...
    public function versionList(){
        ...
        if(isset($_GET['version'])){
            $_GET['version'] = urldecode($_GET['version']);

            $cond[]='version like "%'.$_GET['version'].'%"';
        }
        ...
        $count = M('version_update')->where($cond)->count();
        $page = new page($count,10);
        $version = M('version_update')->order('vid desc')->where($cond)->findall($page->limit());
    ...
    public  function uploadVersion(){
        $db = M('version_update');
        $version = $db->where('vid = '.$_GET['id'])->find();
        $this->assign('version',$version);
        $this->display();
    }
    ...
    public function createVersion(){
        $db = M('version_update');
        C('UPLOAD_IMG_DIR', '');
        $upload = new upload(PATH_ROOT . '/uploads/version', array('apk'));
        $info = $upload->upload();
        if ($info) {
            $avatar = $info[0]['path'];
            $_POST['down_url'] = __ROOT__ .'/'.$avatar;
            $_POST['create_time'] = time();
            $_POST['update_time'] = time();
            if($db->add($_POST)){
                $this->success('上传成功','versionList');
            }else{
                $this->error('上传失败');
            }
        }
    ...
    public function deleteVersion(){
        $id = $_GET['vid'];
        if(M('version_update')->where(['vid'=>$id])->del()){
            $this->success('已删除');
        }else{
            $this->error('未成功，仍需努力');
        }
