...
class dataModelControl extends myControl {
    ...
    /**
     * @Title: isHot
     * @Description: todo(设置热门职位或取消)
     * @author nipeiquan
     * @return  void  返回类型
     */
    function isHot(){
        $laid = $_GET['laid'];
        $link = M('linkage')->where('laid=' . $laid)->find();
        if($link['ishot']==1){
            M('linkage')->where('laid=' . $laid)->update(array('ishot'=>0));//0不是热门1是热门
        }else{
            M('linkage')->where('laid=' . $laid)->update(array('ishot'=>1,'hot_time'=>time()));
        }
        $this->success('操作成功');
    }
    ...
    function cons(){
        ...
        $upload=new upload(PATH_ROOT.'/uploads/cons',array('jpg','jpeg','png','gif'));
        $info=$upload->upload();
        if($info){
            $con=$info[0]['path'];
        }
        if(M('linkage')->where('laid='.$_POST['laid'])->update(array('img'=>__ROOT__.'/'.$con))){
            $this->success('上传成功');
        }else{
            $this->error('上传失败');
        }
    ...
    public function generateClassHtml(){
